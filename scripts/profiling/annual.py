...
def profile_single_file(file_path: str, reports_dir: str):
    ...
    save_metric(
        year_dir,
        "record_count.json",
        {"row_count": row_count, "load_time_seconds": round(load_time, 2)},
    )
    ...
    cat_cols = [
        ...
        if info["logical_type"] == "categorical" or df[col].dtype == "object"
    ]
    ...
    rules = {
        ...
        "logical_constraints": [
            {"name": "registration_date_after_model_year", "expression": "`Año Modelo` <= 2027"}
        ],
    }
    ...
    if "Mes Adquisición" in df.columns:
        df_temp = df.copy()
        df_temp["date_dummy"] = pd.to_datetime(
            "2020-" + df_temp["Mes Adquisición"].astype(str).str.zfill(2) + "-01",
            format="%Y-%m-%d",
            errors="coerce",
        )
