...
Before routing is considered, the selection step alone requires choosing a subset of vehicles from the candidate pool $M$. 

Because the total candidate pool $M$ exceeds the capacity limit of the fleet, the selected subset must be a **non-empty proper subset** of $M$ (i.e., we must deliver at least 1 vehicle, and we cannot deliver all $M$ vehicles):
* $\emptyset \subset S \subsetneq M$
