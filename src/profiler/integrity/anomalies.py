    ...
    Example rules:
    {
      ...
      "logical_constraints": [
         {"name": "registration_date_after_model_year",
          "expression": "registration_year >= anio_modelo"}
      ]
    }
    """
        ...
        for col, bound in rules["value_bounds"].items():
            ...
            if col_violations > 0:
                bounds_violations[col] = {
                    ...
                    "violations_percentage": round((col_violations / total_records) * 100, 4)
                    if total_records > 0
                    else 0.0,
                }
            ...
            try:
                ...
                if violations_count > 0:
                    constraint_violations[name] = {
                        ...
                        "violations_percentage": round((violations_count / total_records) * 100, 4)
                        if total_records > 0
                        else 0.0,
                    }
            except Exception as e:
                constraint_violations[name] = {"expression": expr, "error": str(e)}
